...
metadata:
  hermes:
    tags: [project-lifecycle, bootstrap, adoption, codex, harness]
    related_skills: [codex-harness]
---
...
Own project entry and readiness only. Do not absorb or duplicate the `codex-harness` Stage/Slice engine. Once the lifecycle state is `HARNESS_READY`, delegate execution to `codex-harness`.
...
- Active or interrupted work: follow `codex-harness` recovery; never launch duplicates.
